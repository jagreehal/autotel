...
function formatSpan(span: ReadableSpan, depth: number): string {
  ...
  // What autotel-bedrock adds: the foundation model, Bedrock's own stop
  // reason, and the guardrail outcome.
  const model = a['gen_ai.request.model'];
  if (model !== undefined) detail.push(String(model));
  const stop = a['aws.bedrock.stop_reason'];
  if (stop !== undefined) detail.push(`stop ${String(stop)}`);
  if (a['aws.bedrock.guardrail.intervened'] === true) {
    detail.push(
      `guardrail ${String(a['aws.bedrock.guardrail.id'] ?? 'intervened')}`,
    );
  }
