...
const triageAgent = trace<[string], Promise<TriageResult>>(
  'agent.triage',
  (ctx: TraceContext) =>
    async (userRequest: string): Promise<TriageResult> => {
      ctx.setAttributes({
        'agent.role': 'triage',
...
const specialistAgent = trace<[string], Promise<SpecialistResult>>(
  'agent.specialist',
  (ctx: TraceContext) =>
    async (plan: string): Promise<SpecialistResult> => {
      ctx.setAttributes({
...
const qaAgent = trace<[string], Promise<QAResult>>(
  'agent.qa',
  (ctx: TraceContext) =>
    async (response: string): Promise<QAResult> => {
      ctx.setAttributes({
        'agent.role': 'qa',
        'agent.model': 'gpt-4o',
        'agent.provider': 'openai',
...
>(
  ...
  (ctx: TraceContext) =>
    async (userRequest: string, userId: string): Promise<MultiAgentResult> => {
      ...
      if (triage.requiresSpecialist) {
        ...
        console.log(
          `   ✓ Response generated (quality: ${specialistResult.qualityScore})`,
        );
      } else {
      ...
      if (specialistResult?.requiresQA) {
        ...
        console.log(
          `   ${qaResult.approved ? '✓' : '✗'} QA ${qaResult.approved ? 'Approved' : 'Rejected'} (score: ${qaResult.score})`,
        );
      } else {
      ...
      const agentsInvolved =
        1 + (specialistResult ? 1 : 0) + (qaResult ? 1 : 0);
...
async function main() {
  ...
  await runMultiAgentWorkflow('How do I reset my password?', 'user-456');
