...
const KNOWLEDGE_BASE: VectorSearchResult[] = [
  {
    id: '1',
    content:
      'The company was founded in 2020 and focuses on cloud infrastructure.',
    score: 0.95,
  ...
  },
  {
    id: '2',
    content:
      'Our product offers automatic scaling, monitoring, and deployment tools.',
    score: 0.89,
...
const assembleContext = trace<
  [VectorSearchResult[]],
  Promise<AssembledContext>
>(
  'rag.context_assembly',
  (ctx: TraceContext) =>
    async (results: VectorSearchResult[]): Promise<AssembledContext> => {
...
const generateWithContext = trace<[string, string], Promise<GenerationResult>>(
  'rag.generate',
  (ctx: TraceContext) =>
    async (query: string, context: string): Promise<GenerationResult> => {
      ctx.setAttributes({
...
>(
  ...
  (ctx: TraceContext) =>
    async (
      ...
      console.log(
        `   ✓ Response: "${generation.response.substring(0, 80)}..."`,
      );
