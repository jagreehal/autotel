...
if (!endpoint) {
  ...
  console.error(
    '   Get it from: Grafana Cloud → your stack → Connections → OpenTelemetry → Configure',
  );
  process.exit(1);
}
...
logger.info(
  { endpoint },
  'Autotel initialized for Grafana Cloud (traces, metrics, logs)',
);
...
const createUser = withTracing({})(
  (ctx: TraceContext) => async (name: string, email: string) => {
