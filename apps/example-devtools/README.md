...
- Every reusable traced business function uses `instrument({ key, fn })` for a stable span name. Explicit keys keep the waterfall readable and survive bundler minification. Use `getActiveTraceContext()` inside a plain function, or `withTracing({ name })` when injected context is clearer.
