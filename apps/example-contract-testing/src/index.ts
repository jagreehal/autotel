...
/** The slice of a Pact document this example rewrites. */
type PactDocument = {
...
function injectStalePactFile(): void {
  ...
  // SAFETY: this file is a pact document this same script wrote a moment ago;
  // only `messages` is read, and the line below defaults it when absent.
  const existing = JSON.parse(readFileSync(path, 'utf8')) as PactDocument;
