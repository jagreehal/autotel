---
title: Architecture
description: Code structure patterns, conventions, and important architectural decisions.
---
...
ambiguous and nothing inspects a callback's parameter name.
...
- Production/minified reusable functions should use `trace(name, fn)`, `instrument({ key, fn })`, or `withTracing({ name })` for stable names
...
- Works in both CJS and ESM builds (tsdown handles the differences)
...
export const postCheckout = withTracing({})((ctx) => async (req, res) => {
...
- [`example-canonical-logs`](https://github.com/jagreehal/autotel/tree/main/apps/example-canonical-logs): Canonical log lines (wide events) vs scattered logging.
- [`example-effect`](https://github.com/jagreehal/autotel/tree/main/apps/example-effect): Effect v4 with [`autotel-effect`](https://github.com/jagreehal/autotel/tree/main/packages/autotel-effect) for OTel export.
- [`awaitly-example`](https://github.com/jagreehal/autotel/tree/main/apps/awaitly-example): awaitly workflows with autotel instrumentation and Mermaid visualization.
...
- [`example-bunyan`](https://github.com/jagreehal/autotel/tree/main/apps/example-bunyan): Bunyan with auto-instrumentation for trace context injection.
- [`example-winston`](https://github.com/jagreehal/autotel/tree/main/apps/example-winston): Winston with auto-instrumentation for trace context injection.
- [`example-otel-tui`](https://github.com/jagreehal/autotel/tree/main/apps/example-otel-tui): otel-tui terminal UI with Docker Compose for trace-to-log correlation.

### Collector Examples

- [`example-collector-pipeline`](https://github.com/jagreehal/autotel/tree/main/apps/example-collector-pipeline): OTTL redaction, health-check filtering, count-before-sampling, and tail sampling in front of the backend.
- [`example-genai-metrics`](https://github.com/jagreehal/autotel/tree/main/apps/example-genai-metrics): canonical GenAI metrics derived from `gen_ai.*` spans with the `signal_to_metrics` connector, charted in Grafana LGTM.
- [`example-genai-evals`](https://github.com/jagreehal/autotel/tree/main/apps/example-genai-evals): brevity, groundedness and prompt-injection evaluators scoring sampled answers into `gen_ai.evaluation.result` events, with an alertable pass rate in Loki.
...
- Use `tsdown` for bundling (not tsc directly)
