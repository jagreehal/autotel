...
- Gracefully handles circular references (→ `<circular-reference>`)
...
A flagged rollout splits your traffic into two populations that share a service
name, a route and a version. Without the flag on the span there is no way to ask
the only question that matters during a rollout — is the new branch slower, or
failing more, than the old one — and the usual workaround is to read it in the
flag vendor's dashboard, where the latency and the errors are not.

`autotel/feature-flags` records evaluations under the OpenTelemetry convention,
so any backend can split any metric by variant with no vendor in the query path.

```typescript
import { recordFeatureFlag } from 'autotel/feature-flags';

export const checkout = trace((ctx) => async (userId: string) => {
  const useNewFlow = await flags.get('new-checkout');
...
Using OpenFeature? The hook is the zero-config path — it already sits between
your application and the vendor, and it already fires at the moment the code
branched:
...
**Record where the code reads the flag, not where it is fetched.** A flag
evaluated and ignored explains nothing; the value the request actually branched
on is the one that explains its behaviour.

### Attributes and events both

Each evaluation writes `feature_flag.key`, `feature_flag.result.value`,
`feature_flag.result.variant`, `feature_flag.result.reason`,
`feature_flag.provider.name` and `feature_flag.context.id`, and emits one
`feature_flag.evaluation` event.

Both, because span attributes hold a single flag — a second call overwrites the
first — while a request that branched on three flags needs one event each.
Attributes keep a single-flag span filterable without unpacking events; the
events keep a many-flag span complete.

Values keep their type: a boolean flag records `true`, not `"true"`. Reasons are
normalised to the registry's lower snake case, so a provider reporting
`TARGETING_MATCH` and one reporting `targeting_match` land in the same bucket. A
failed evaluation is still recorded, with the default value the code actually
used and `reason: 'error'` — the default is what the request behaved as, and the
failure is why.

Never key an attribute by flag name (`feature_flag.new-checkout`): it cannot be
...
log.set({ 'payload.size_bucket': bucket(size, [1024, 65_536]) });
// '<1024' | '1024-65536' | '>=65536'
```

`bucket(value, boundaries)` sorts the boundaries you pass, so an unordered list
...
Deploy timestamps make poor cohort boundaries. You end up reconstructing which
...
await trace.run('checkout', async () => {
  experiment({
    name: 'checkout-cache',
...
Call it at any depth inside a traced body. It reads the ambient span, and it
does nothing when no trace is running.

Your expectation travels with the result, so a reader a month later sees the
claim next to what happened. The devtools [Compare
view](/tools/devtools/#compare-two-cohorts) lists the experiments it has seen
and fills both cohorts from the arms of the one you pick.
