...
Every span the request starts after that call - the route handler, its database
queries, outgoing HTTP - carries `user.id` too, so you can filter on it anywhere
in the trace. A span's own value for the same key wins. The copy stays
in-process: unlike baggage, nothing is sent to downstream services.
