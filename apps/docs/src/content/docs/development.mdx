---
title: Development
description: Development commands, testing patterns, and common workflows for the Autotel monorepo.
---
...
- `createMemoryExporter()`: A `SpanExporter` that collects finished spans as
  plain objects (`name`, `traceId`, `parentSpanId`, `durationMs`, `attributes`,
  `status`) with `spans()`, `findSpan(name)`, `findSpans(name)` and `reset()`.
  Pass it to `init({ spanExporters: [...] })` to read telemetry back the way a
  backend would see it, without hand-writing an exporter against the SDK types.
...
1. Add instrumentation logic to `packages/autotel/src/`
...
- Install new dependency: `pnpm add <package> --filter <workspace-name>` (e.g., `pnpm add zod --filter autotel`)
