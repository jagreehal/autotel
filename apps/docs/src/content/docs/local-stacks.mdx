...
| Langfuse | LLM traces, cost, evaluations | `docker compose -f docker/langfuse.yml up -d` | [http://localhost:3000](http://localhost:3000)   |
...
All three take OTLP straight from your app. Put a [collector](/autotel/collector/) in front once you want to sample whole traces, mask attributes autotel does not recognise, or count requests before anything is dropped.
...
Langfuse turns a `chat` span into a generation, reads `gen_ai.usage.input_tokens` and `gen_ai.usage.output_tokens` as usage, and prices the call from its own model table. Sign in at [http://localhost:3000](http://localhost:3000) as `dev@example.com` / `localdevpassword`.

:::caution
Every credential in `docker/langfuse.yml` is a local development value, including the encryption key. Regenerate all of them before the stack goes anywhere but your machine.
:::
