...
- `OTEL_EXPORTER_OTLP_PROTOCOL`: `http/json`, `http/protobuf`, or `grpc` (`http` remains an alias for JSON)
...
init({
  ...
  endpoint: 'http://localhost:4318', // Single OTLP destination shorthand
});
```

For multi-backend OTLP fan-out, use `destinations` instead of overloading `endpoint`:
...
With no `endpoint` from `init()`, YAML or `OTEL_EXPORTER_OTLP_ENDPOINT`, there is
**no default and no fallback to localhost**. The processor lists stay empty and
nothing is exported.

This is worth stating because the OpenTelemetry Node SDK does the opposite:
leave the processors unset and it installs its own OTLP exporter aimed at
`http://localhost:4318`. On a server with nothing listening there, that is a
doomed request per batch, forever, with no error that names the cause — so
"no endpoint configured" would quietly become "export to localhost". Autotel
passes empty processor lists for both traces and logs instead.

Two consequences worth knowing:

- `spanProcessors: []` is a real off switch. Empty and absent are different
  things: an empty array means "no processors", and it wins over the endpoint
  pipeline autotel would otherwise build.
- `OTEL_LOGS_EXPORTER` still governs itself. Set it and autotel stays out of the
  way, because that is the specification's own switch and someone who set it
  means it.

Point `endpoint` at a local collector, or set `devtools: true`, to send there on
purpose.

## OTLP wire protocol

Autotel keeps the two OTLP/HTTP encodings distinct:

| `init()` value  | Environment value | Wire format        | Trace exporter                                             |
| --------------- | ----------------- | ------------------ | ---------------------------------------------------------- |
| `http`          | `http/json`       | OTLP/HTTP JSON     | `@opentelemetry/exporter-trace-otlp-http` (included)       |
| `http/protobuf` | `http/protobuf`   | OTLP/HTTP protobuf | `@opentelemetry/exporter-trace-otlp-proto` (optional peer) |
| `grpc`          | `grpc`            | OTLP/gRPC          | `@opentelemetry/exporter-trace-otlp-grpc` (optional peer)  |

`OTEL_EXPORTER_OTLP_PROTOCOL=http` is accepted as the long-standing JSON
alias. `http/protobuf` is not interchangeable with `http`: it selects the
protobuf exporter and fails clearly at startup if that optional package is not
installed. Install the matching metric and log protobuf exporters as well when
those signals are enabled.
...
HTTP endpoints may include a path prefix. Autotel preserves it when appending
the signal path, so an endpoint ending in `/i` sends traces to `/i/v1/traces`.

## Process Handlers

A container gets `SIGTERM`, your process exits, and the last few seconds of
telemetry never leave the buffer. Importing `autotel` registers no process
listeners, so you opt in:

```typescript
init({
  service: 'my-app',
  processHandlers: true, // flush and exit on SIGTERM/SIGINT and fatal errors
});
```

`true` (or `{}`) installs the defaults. Pass an object to change one of them:
...
init({
  ...
  processHandlers: {
    ...
    fatalErrors: true, // flush on uncaughtException / unhandledRejection
    shutdownTimeoutMs: 2000, // give up waiting and exit anyway
  },
});
```

Autotel prints fatal errors to stderr before flushing. Registering an
`uncaughtException` listener overrides Node's default stack trace, so without
that print a crash under `fatalErrors` would exit in silence.

Leave `processHandlers` off when your application already owns its lifecycle.
Applications with their own resource cleanup should call `shutdown()` from their
...
process.on('SIGTERM', async () => {
  await Promise.all([closeDatabase(), shutdown()]);
  process.exit(0);
});
```

Two handlers fighting over the same signal is the failure mode this avoids.
...
Span processors that decorate spans on the way out. Unlike `spanProcessors`,
which **replaces** the pipeline autotel builds from `destinations`, enrichers are
**added** to it:

```typescript
init({
  service: 'support-agent',
  destinations: [{ endpoint: 'https://collector.example.com' }],
  spanEnrichers: [langfuseCompatibility({ tags: ['production'] })],
});
```

Use them for anything that adds attributes rather than exporting: mapping
canonical attributes onto a backend's own names, stamping a deployment id,
deriving one attribute from another. A processor passed as `spanProcessors`
would take the pipeline over and silently switch off every destination above it.

Enrichers run first, and outside the redaction, normalizer and filter wrappers.
That ordering is deliberate: they decorate the real span, and
[Attribute Redaction](#attribute-redaction) downstream then sees the attributes
they added rather than missing an enricher that copied a sensitive value into a
new key.
...
For redaction driven by portable rule files rather than presets, see
[Telemetry Policies](/autotel/policies/).

## Telemetry Policies

Portable, fail-open rules describing what telemetry to keep and how to transform
it, following [OTEP 4738](https://github.com/open-telemetry/opentelemetry-specification/blob/main/oteps/4738-telemetry-policy.md).
...
The path is watched, so policies can change without restarting the process. See
[Telemetry Policies](/autotel/policies/) for the supported subset of the
specification.
...
# Telemetry Policies: a .json file or a directory of them, watched for changes
# policies: ./policies
...
**Production (Grafana + Honeycomb fan-out via `init()`):**
