...
| `tracer.startSpan()` + `span.end()`                                           | `trace(fn)` or `trace(name, fn)`         | Automatic span lifecycle            |
...
| `tracer.startSpan()` + `span.end()` | `trace(fn)`, `trace(name, fn)`   | Automatic lifecycle         |
...
- [ ] Replace with the `trace(fn)` / `trace(name, fn)` functional wrapper
