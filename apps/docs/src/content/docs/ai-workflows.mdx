...
| `autotel-genai/observer` | `createGenAiObserver()` event-stream adapter, with `createLangChainObserver()` and `observeAiSdkResult()` glue |
...
Two adapters ship with the observer:
