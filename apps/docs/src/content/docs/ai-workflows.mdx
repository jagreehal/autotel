...
| `autotel-genai/observer`  | `autotelTelemetry()` for `registerTelemetry()`, `subscribeAiTelemetry()` for the `ai:telemetry` channel, plus `createGenAiObserver()`, `createLangChainObserver()`, `createMastraObserver()`, and `observeAiSdkResult()` |
...
`traceGenAI()` wraps code you own. When a framework emits its own telemetry
stream, subscribe `createGenAiObserver` and feed every event through it. It
rebuilds the `gen_ai.*` span tree and force-closes any child whose terminal
event never arrives.

Token usage and cost land on leaf `chat` spans. Aggregate `agent` and
`workflow` spans carry no `gen_ai.usage.*`, so a backend that sums usage across
a trace counts each call once.
...
`autotelTelemetry()` emits canonical `gen_ai.*` spans as calls run, prices every
model call, records streaming timing, and nests provider HTTP spans plus nested
tool-triggered `generateText()` calls under the right parent. The channel
subscriber gives you the same tree with usage and cost, but not per-call
streaming timing.

The event-stream adapters are still useful when you already have a finished AI
SDK result or another framework lifecycle stream:

```typescript
import {
  createGenAiObserver,
  createLangChainObserver,
  createMastraObserver,
  observeAiSdkResult,
...
// Mastra: an exporter on its observability pipeline.
new Mastra({
  agents,
...
Prompts, tool arguments, and tool results stay off spans by default. Pass an
`exportContent` callback to opt in and redact what reaches each span.

The `example-langchain-observer` app runs a LangGraph agent on a local Ollama
model and prints the captured tree, token counts, and tool arguments.

### Mastra

`createMastraObserver()` maps Mastra's agent runs, model generations, RAG
embeddings, tool calls, and workflow steps onto the observer's events. Mastra
plumbing — model steps and chunks, processors, scorers, mappings — is dropped
and its children reparent to the nearest kept ancestor. Pass `skipSpan` to drop
additional supported span types; unsupported plumbing always stays dropped.
Usage lands on the `model_generation` span (`model_step` /
`model_inference` are dropped precisely so `gen_ai.usage.*` is not counted
twice), and cost is estimated from it.

`@mastra/otel-exporter` also emits canonical `gen_ai.*`, so pointing it at an
OTLP endpoint works. Use this adapter instead when you want Mastra's spans to be
_autotel's_ spans: Mastra dispatches its events synchronously, so the agent run
nests inside the request span you already opened, flows to every `init()`
destination, passes through your `spanEnrichers` (including
`langfuseCompatibility`), and gets priced — none of which a separate exporter
with its own endpoint and its own span processor can do.
...
Streaming latency is two numbers, not one: **time to first chunk** (the wait
before anything appears) and **throughput** (how fast tokens then arrive). A
single duration hides both. `createStreamTimer` from `autotel-genai/streaming`
captures the full picture and records the headline values as canonical
...
inter-chunk gap distribution `{ min, p10, median, avg, p90, max }` (seconds) for
diagnosing token-arrival jitter.

## Pattern: Budgets & Guardrails

A guard runs _during_ a run, not after the bill. Feed it each step (an LLM call,
a tool call, a delegation) and it accumulates cost / tokens / loop state, then
halts the run when a rule crosses its threshold. Aborting an `AbortSignal` and
(by default) throwing a `GEN_AI_GUARD_STOP` structured error. All deterministic,
no LLM in the loop.
...
Rules can also come from a shorthand string. Cost/token ceilings, spin-loop
detection (`N identical calls in a window of M`), error loops, tool-call/step
caps, wall-clock timeouts, and context-window budgets:
...
Each rule fires once. Thread `guard.signal` into your model/tool calls for
cooperative cancellation, or catch `GEN_AI_GUARD_STOP` to wind the run down.

## Pattern: Evaluation Loops

Implement quality checks and iterative refinement with full observability.

:::note[Scoring live traffic]
`recordEvaluationResult` writes a `gen_ai.evaluation.result` event on the trace
of the call it judges, which turns pass rate into a query you can alert on.
[`example-genai-evals`](https://github.com/jagreehal/autotel/tree/main/apps/example-genai-evals)
runs three model-free evaluators over a sample of answers and charts the result
in Grafana. Events reach a backend through a subscriber, so configure one or
`recordEvaluationResult` is a no-op.

For conversation signals at production volume (frustration, follow-up,
disagreement, resolved, agent corrected), use `runConversationSignals` with an
evaluation model such as TypeSafe Jev. One shared conversation state, several
independent boolean questions in one pass, each becoming a
`gen_ai.evaluation.result` with a yes/no label from your threshold:

```typescript
import { runConversationSignals } from 'autotel-genai';

// Pin a versioned id (not jev-latest) when wiring a real evaluation model.
...
// results: [{ name: 'user_frustrated', scoreValue, scoreLabel: 'yes'|'no' }, ...]
```

`EVAL_MODE=signals pnpm start` in the example app demos the same path with an
in-process fake model (no API key).
:::
...
:::note[Metrics from these spans]
`autotel-genai` writes the attributes and leaves the metric instruments to you.
A collector can derive the canonical GenAI metrics from these spans instead, so
cost per model, error rate, time to first chunk and tool usage need no extra
application code. See [Derive the GenAI
metrics](/autotel/collector/#derive-the-genai-metrics).
:::
...
const generate = traceGenAI({
  ...
  recordGenAiResponse(ctx, {
    model: res.model,
    finishReasons: res.finishReasons,
  });
