...
Use `autotel/workers` from the main `autotel` package for primary DX. It wraps `fetch` so each request gets a span; use `getRequestLogger()` or trace context inside the handler. Use `autotel-cloudflare` when you need feature-targeted Cloudflare instrumentation surfaces (bindings, handlers, actors, agents, workflows).

### Audit / Compliance

For audit-grade events that must bypass tail-drop sampling, use the optional
