...
Do not guess which handlers are dark. Read them:

```bash
npx autotel map --json --no-write                  # every entry point
npx autotel map --json --no-write <route|file>     # one entry point
```

Each entry in `map.routes[].checks` carries a `status`, a `message`, `evidence`
(`file`, `line`, `snippet`), and a `fix`. Work from those. The checks land on
the table below: `trace` wants a span, `context` wants a request logger with
attributes, `structured-errors` wants `createStructuredError` with `why` and
`fix`, `error-handling` wants catch blocks that record, `audit` wants
`withAudit()` on money and auth paths.

`autotel.map.json` gets committed, so the score is trackable. A check that
should not apply gets waived in code with
`// autotel-map-disable <check> -- reason` rather than by lowering the CI
threshold. See [Observability Map](/autotel/tools/map/).
...
| Run named work now with context    | `trace.run('checkout', async (ctx) => { ctx.setAttribute(...); ... })`      | One-off operations, run right here       |
| Reusable handler with context      | `withTracing({})((ctx) => async (args) => { ctx.setAttribute(...); ... })`  | Wrapped functions that need context      |
...
| Same error thrown from many places | `defineErrorCatalog('billing', { PAYMENT_DECLINED: { … } })`                | Repeated failures, `.match()` in catches |
| Set snapshot severity explicitly   | `log.setLevel('warn')`                                                      | Degraded response with nothing to log    |
