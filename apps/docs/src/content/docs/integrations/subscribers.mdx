...
description: Send product events to Mixpanel, Amplitude, Segment, webhooks, and Slack — and to PostHog via autotel-posthog.
---
...
- PostHog product events → `PostHogSubscriber` from `autotel-posthog/subscriber` (server `track()` only; the browser session join is [PostHog](/autotel/integrations/posthog/))
...
npm install autotel-posthog posthog-node    # For PostHog
...
| Subscriber           | Import                                      |
| -------------------- | ------------------------------------------- |
| PostHog              | `autotel-posthog/subscriber`                |
...
| Grafana Loki         | `autotel-subscribers/loki`                  |
...
Each observation also carries `sources`: one entry per distinct
`(producer, channel)` pair, with that pair's own count and first/last-seen. Read
`sources` when attributing a count to a specific relationship, so two services
publishing the same event name each keep their own traffic. The list is
additive and sorted, so existing readers are unaffected and a committed
...
init({
  service: 'my-service',
  subscribers: [
    createMixpanelSubscriber({ token: 'mp_...' }),
    createWebhookSubscriber({ url: 'https://example.com/events' }),
  ],
});
```

Factories: `createMixpanelSubscriber`,
...
- [`example-posthog`](https://github.com/jagreehal/autotel/tree/main/apps/example-posthog): browser join plus a checkout that fails, so you can see the replay URL and `$trace_id`.

## Entry Points

- `autotel-subscribers`: Base class + re-exports
- `autotel-posthog/subscriber`: PostHog
...
- `autotel-subscribers/loki`: Push events to Grafana Loki ([Grafana Loki](/autotel/integrations/loki/))
