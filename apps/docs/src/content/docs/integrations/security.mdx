...
description: Use Autotel as a security control plane — emit redaction-safe security events with a stable schema, derive signals from telemetry you already have, alert on rates and absences, and triage incidents from the CLI.
---

Use Autotel as a security control plane: emit security events with a stable
schema, derive signals from telemetry you already have, alert on rates and
absences, and triage incidents from the terminal.

:::note[Why this exists]
OWASP A09:2025 (Security Logging & Alerting Failures) calls out missing logs
for important events, unclear messages, and ineffective alerting. Autotel's
answer: structured, correlated, redaction-safe, sampling-exempt security
telemetry plus alertable metrics. Detection rules and dashboards stay in your
backend, where they belong — this feature's job is to make the signals exist,
survive sampling, and stay queryable under a stable schema.
:::
...
logger. Events are **force-kept through tail sampling by default** — an attack
you sampled away is an attack you cannot investigate — and carry the stable
`security.*` attribute schema so backends can build detection rules from it.
...
- **LLM consumption anomalies** (OWASP LLM10) — a per-call token ceiling plus
...
A silently-dead telemetry pipeline is itself a security failure.
...
and sample trace IDs for pivoting — one JSON document on stdout. See the
[CLI reference](/autotel/tools/cli/) for the full investigate command set.
...
And periodically fire a synthetic probe (e.g. request `/.env` from a canary)
to verify the whole chain: signal → metric → alert → human.
