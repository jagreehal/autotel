...
import { init, type TraceContext, withTracing } from 'autotel';
...
export const getUserWithPosts = withTracing({ name: 'getUserWithPosts' })(
...
export const publishPost = withTracing({ name: 'publishPost' })(
 ...
 */
export const slowSearch = withTracing({ name: 'slowSearch' })(
...
/** One database operation the driver loop runs for its spans; results are discarded. */
type DriverOp = () => Promise<void>;

function pickFastOp(userId: number): DriverOp {
  const ops: DriverOp[] = [
    async () => {
    ...
    async () => {
      await publishPost(Math.max(1, Math.floor(Math.random() * 10)));
    },
  ];
