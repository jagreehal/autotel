...
async function fetchUserFromDb(userId: string) {
  return trace.run('db.users.find', async (ctx) => {
...
async function checkInventory(items: string[]) {
  return trace.run('inventory.check', async (ctx) => {
...
async function chargePayment(amount: number) {
  return trace.run('payment.charge', async (ctx) => {
...
async function sendConfirmationEmail(orderId: string) {
  return trace.run('email.send', async (ctx) => {
...
async function processOrder() {
  return trace.run('order.process', async (ctx) => {
...
async function healthCheck() {
  return trace.run('health.check', async (ctx) => {
...
async function main() {
  ...
  for (let i = 0; i < 3; i++) {
    try {
      ...
      console.log(
        `  ❌ Order ${i + 1}: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
    await delay(200);
  }
