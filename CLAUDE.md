...
- `packages/autotel-effect` - Effect v4 bridge: `layer({ serviceName })` provides Effect's `Tracer` from the global provider `autotel.init()` registers, so `Effect.withSpan` spans export through autotel. Wraps `@effect/opentelemetry` only; no tracer of its own (see `packages/autotel-effect/README.md`)
