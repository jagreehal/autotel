...
- `packages/autotel-bedrock` - Amazon Bedrock facts for `gen_ai.*` spans: `bedrockCompatibility()` enricher (foundation-model id from inference profiles and ARNs, `cloud.region`), `bedrockProviderAttributes` for `autotelTelemetry({ providerAttributes })` (Bedrock's own stop reason, guardrail id and outcome), `BEDROCK_PRICING` for Bedrock-only models. `autotel-genai` emits the canonical span; this package adds what no convention covers (see `packages/autotel-bedrock/README.md`)
...
- `packages/autotel-agents` - Coding-agent observability: browser-safe domain layer that turns the OTel metrics + log events from Claude Code / opencode / Codex into a session model (adapter registry, reducers, MCP/sub-agent/skill taxonomy), with spend attributed by model, effort, skill, sub-agent and prompt. Consumed by the autotel-devtools Agents tab, which also enables Claude Code's span hierarchy and renders it in Traces (see `packages/autotel-agents/README.md`)
- `packages/autotel-effect` - Effect v4 bridge: `layer({ serviceName })` provides Effect's `Tracer` from the global provider `autotel.init()` registers, so `Effect.withSpan` spans export through autotel. The same call bridges `Effect.log*`, emitting each log as an OpenTelemetry log record plus a trace-correlated stdout line (`logs: false` for spans only; `loggerLayer()` is exported for logs alone). Wraps `@effect/opentelemetry` only; no tracer of its own (see `packages/autotel-effect/README.md`)
...
- `packages/autotel-claude-code` - Claude Code mod (function hooks): `$.autotel.span` for plugin authors, one span per hook dispatch with the chain beneath it, a trace per interaction, tool-call duration on the row. Runs in Claude Code's hook sandbox, so no OTel SDK: plain OTLP/JSON over `$.http.fetch`. Hooks are typed against the vendored `types/claude-code.d.ts` (see `packages/autotel-claude-code/README.md`)
