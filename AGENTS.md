...
- **Request-wide attributes**: `requestCtx` is `ctx` aimed at the request span, for context that describes the whole request (user, tenant, plan) set from a shared middleware. `ctx` stays pointed at the span the code is in. Outside a request `requestCtx` falls back to the active span.
