...
- **Rich attribute values**: `ctx.setAttribute()` / `setAttributes()` take objects, `Map`s, `Set`s and `Date`s and flatten them to dot-notation keys, as the request logger and structured errors do. Never hand-flatten before the call.
- **`ctx` off-span**: The ambient `ctx` no-ops when no span is active (uninstrumented test run, `node server.js` with nothing preloaded), so instrumentation can never be what throws. `ctx.traceId` still reads `undefined` — guard on that, not on the methods.
- **Request-wide attributes**: `requestCtx` is `ctx` aimed at the request span, for context that describes the whole request (user, tenant, plan) set from a shared middleware; spans the request starts afterwards inherit them (in-process, never baggage; a span's own key wins). `ctx` stays pointed at the span the code is in. Outside a request `requestCtx` falls back to the active span.
- **Express middleware**: `@opentelemetry/instrumentation-express` runs each layer under a span of its own, so an attribute set from a middleware would land on a `middleware - …` span that ends at `next()`. autotel defaults express to `ignoreLayersType: ['middleware', 'request_handler']`, which puts them on the request span; `express: { ignoreLayersType: [] }` restores layer spans (options beyond `enabled` reach the instrumentation constructor). Never suggest `applyCustomAttributesOnSpan` for this.
...
│   ├── autotel-effect/        # Effect v4 bridge: layer() provides Effect's Tracer, withAutotel() joins the surrounding trace
│   ├── autotel-vitest/        # Vitest fixture: one span per test, trace annotation on the report
│   └── ...
├── docs/
│   ├── DEVELOPMENT.md
│   ├── ARCHITECTURE.md
...
- **Stable names in examples**: Runnable examples must not emit `unknown` span names. Use `trace('operation.name', fn)` for a reusable function, or `trace.run('operation.name', ctx => result)` for immediate work, when inference is not guaranteed. Examples reach the span through the ambient `ctx` import rather than a threaded parameter, so the code a reader copies is the shape this file recommends. `apps/book-chapters` and the `.claude/skills/autotel` snippets follow both rules; keep new examples on the same two forms.
...
| **Browser / SPA**      | `autotel-web`: `init()` injects `traceparent` on same-origin requests; name cross-origin APIs in `propagateTo` and have them allow the header. For per-tenant tracing, call `setBaggage({ 'tenant.id': id })` after login: it propagates as a W3C `baggage` header (same-origin/fail-closed) and the backend's `BaggageSpanProcessor` tags server spans. Do **not** hand-roll a fetch wrapper for this. |
| **Effect v4**          | `autotel-effect`: `layer({ serviceName })` once in the layer graph, then `Effect.runPromise(withAutotel(program))` wherever the run sits inside an autotel span, so Effect spans join that trace. Instrument effects with `Effect.fn` / `Effect.withSpan`, never `trace()`.                                                                                                                             |
| **Tests**              | `autotel-vitest` / `autotel-playwright`: one span per test, `traceparent` injected into requests to the API, and each test annotated with its trace so the report links back to it.                                                                                                                                                                                                                     |
...
- **Request attribute inheritance**: Retain attribute scopes, never ancestor span objects. Flatten each `requestCtx` setter input once and reuse the result for the span and its descendants; getters and custom serializers must not run twice.
...
- [ ] No `trace(name, fn)` around a function that returns an `Effect` — the span would end when the effect is built. Use `Effect.fn` / `Effect.withSpan`.
- [ ] `trace(name, fn)` returns a wrapper; it has to be called. `trace.run(name, fn)` is the form that runs now.
