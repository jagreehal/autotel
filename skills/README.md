...
| [`autotel-bedrock`](integrations/autotel-bedrock/SKILL.md)                         | Amazon Bedrock facts on autotel's gen_ai.* spans — bedrockCompatibility() to normalise inference-profile ids and ARNs to the foundation model, bedrockProviderAt… |
| [`autotel-builtin-ai`](integrations/autotel-builtin-ai/SKILL.md)                   | Tracing Chrome's built-in AI APIs — the on-device model a page runs through LanguageModel, Summarizer, Writer and friends: availability, session creation and pe… |
...
| [`autotel-effect`](integrations/autotel-effect/SKILL.md)                           | Wiring Effect v4 to autotel so Effect.withSpan spans export through the same OpenTelemetry pipeline — the layer() bridge, init ordering with --import, and what…  |
