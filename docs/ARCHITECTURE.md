...
**`trace` wraps, `trace.run` runs.** Every `trace(...)` form returns a
wrapper and executes nothing, so importing a module full of them has no
side effects. `trace.run(...)` is the only form that runs the operation.
...
| `trace(name)(fn)`           | wrapper, curried           | applying one config to many fns  |
| `trace.run(name, ctx => r)` | the operation's result     | one operation, run right here    |
| `instrument({ key, fn })`   | wrapper, name explicit     | the options form of the wrapper  |
| `withTracing(opts)(f)`      | wrapper from a ctx factory | reusable config, ctx in scope    |

```typescript
import { trace, ctx } from 'autotel';

// Reusable, name inferred from the binding
export const getUser = trace(async (id) => {
  return await db.users.findById(id);
});

// Reusable, explicit name — reach the span through the ambient ctx
export const createUser = trace('user.create', async (data) => {
  ctx.setAttribute('user.id', data.id);
  return await db.users.create(data);
});

// One operation, run right here
const user = await trace.run('user.create', async (ctx) => {
  ctx.setAttribute('user.id', input.id);
  return await db.users.create(input);
});

// Factory pattern (ctx bound for the whole wrapper)
export const updateUser = withTracing({})((ctx) => async (data) => {
  ctx.setAttribute('user.id', data.id);
  return await db.users.update(data);
});
```

Because `trace` and `trace.run` are separate names, no call shape is
ambiguous and nothing inspects a callback's parameter name. That heuristic is
what [#166](https://github.com/jagreehal/autotel/issues/166) removed after a
minifier renamed `ctx` and crashed deployed Lambdas.

Inside any traced body, the ambient `ctx` import resolves to the active span —
including in helpers several frames down, which a context parameter cannot
reach without being threaded through every call.
...
1. **Explicit name** (from an immediate `trace('customName', operation)`, a `withTracing({ name })` factory, or an `instrument()` key)
...
- Production/minified reusable functions should use an explicit stable name with `trace(name, fn)`, `instrument({ key, fn })`, or `withTracing({ name })`. Development emits one diagnostic if resolution still falls back to `unknown`.
