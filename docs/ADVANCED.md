...
- Descends into nested objects and `Map`s alike, giving dot-notation keys
- Reads a `Set` as the array it carries
- Auto-serializes remaining non-string values to JSON
- Filters out null/undefined values
- Handles circular references (→ `<circular-reference>`)
