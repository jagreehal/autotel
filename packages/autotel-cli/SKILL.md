...
npx autotel-cli schema

# Error envelope shape and stable AUTOTEL_E_* codes
npx autotel-cli schema errors

# JSON output shapes per command
npx autotel-cli schema outputs

# Compact one-line-per-command listing
npx autotel-cli commands

# Copy-pasteable examples
npx autotel-cli examples              # all
npx autotel-cli examples init         # one command

# Version + runtime info
npx autotel-cli version
```

Every command supports `--json`. Errors are returned as a structured envelope
(see `npx autotel-cli schema errors`). Exit codes:
...
npx autotel-cli map --json --no-write

# One entry point in detail
npx autotel-cli map --json --no-write app/api/checkout/route.ts

# Gate: exit 1 below a score, or when a passing check regresses
npx autotel-cli map --min-score 70 --json
npx autotel-cli map --baseline git:origin/main --json
```
...
npx autotel-cli init --detect-only --json

# Preview as JSON without writing
npx autotel-cli init --json --dry-run

# Apply detected plan non-interactively
npx autotel-cli init --yes --json

# Generate a plan, hand it to a human for review, apply later
npx autotel-cli init --detect-only --json --output-file /tmp/plan.json
# ... review /tmp/plan.json ...
npx autotel-cli init --plan /tmp/plan.json --json

# Pipe a plan from stdin
cat plan.json | npx autotel-cli init --input - --json

# Redact secret-shaped values from all JSON output
npx autotel-cli init --json --no-secrets-in-output
