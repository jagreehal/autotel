...
function updateStatusBar(state: ReceiverState): void {
  ...
  if (state === 'running') {
    ...
    statusBarItem.tooltip =
      droppedTraceCount || droppedLogCount
...
function summarizeMetrics(
  currentTraces: readonly TraceData[],
): MetricsSummary[] {
  const byService = new Map<
    string,
    {
      durations: number[];
      errors: number;
      ops: Map<string, { d: number[]; errs: number }>;
    }
  >();
...
function buildServiceMap(currentTraces: readonly TraceData[]): ServiceMapData {
  ...
  for (const trace of currentTraces) {
    ...
    for (const span of trace.spans) {
      const svc = String(
        span.attributes?.['service.name'] ?? trace.service ?? 'unknown',
      );
...
function openServiceMapPanel(): void {
  if (serviceMapPanel) {
    ...
    serviceMapPanel.webview.html = renderServiceMapHtml(
...
function renderServiceMapHtml(data: ServiceMapData): string {
  ...
  const yFor = (svc: string): number =>
    20 + data.services.indexOf(svc) * ROW_HEIGHT + ROW_HEIGHT / 2;
  ...
  const edgePaths = data.edges
    .map((edge) => {
      ...
      const stroke = Math.max(
        1,
        Math.min(8, Math.log2(edge.callCount + 1) * 1.5),
      );
...
async function setBackendCredential(): Promise<void> {
  if (!extensionSecrets) {
    void vscode.window.showErrorMessage(
      'Extension not fully activated; secrets unavailable.',
    );
    return;
  }
  ...
  void vscode.window.showInformationMessage(
    `Saved ${adapterId} credential to VSCode SecretStorage.`,
  );
}
...
async function queryRemoteBackend(): Promise<void> {
  ...
  if (!baseUrl) {
    void vscode.window.showErrorMessage(
      'autotel.backend.url is required to query the backend.',
    );
    return;
  }
  const dataset =
    config.get<string | null>('backend.dataset', null) ?? undefined;
  ...
  await vscode.window.withProgress(
    ...
    async (_progress, token) => {
      ...
      try {
        ...
        if (!service) return;
        const remoteTraces = await adapter.searchTraces(ctx, {
          service,
          limit: 50,
        });
        if (remoteTraces.length === 0) {
          void vscode.window.showInformationMessage(
            `No traces for ${service}.`,
          );
          return;
        }
...
async function ensureReceiverHostAllowed(
  ...
  outputChannel?.appendLine(
    `Receiver start blocked for non-loopback host: ${host}`,
  );
  updateStatusBar('stopped');
  return 'blocked';
}
...
async function revealSource(arg?: unknown): Promise<void> {
  ...
  const zeroBasedLine = Math.max(
    0,
    Number.isFinite(lineno) ? Number(lineno) - 1 : 0,
  );
...
async function openSpanDetail(arg?: unknown): Promise<void> {
  ...
  const trace = traces.find((t) =>
    t.spans.some((s) => s.spanId === span.spanId),
  );
  if (!trace) {
    void vscode.window.showInformationMessage(
      'Trace for span not found in buffer.',
    );
    return;
  }
...
class LogsProvider implements vscode.TreeDataProvider<LogData> {
  ...
  getTreeItem(log: LogData): vscode.TreeItem {
    ...
    item.description =
      `${log.severityText ?? ''} ${log.resourceName ?? ''}`.trim();
    item.iconPath = new vscode.ThemeIcon(
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const tracesView = vscode.window.registerTreeDataProvider(
    'autotel.traces',
    tracesProvider,
  );
  const logsView = vscode.window.registerTreeDataProvider(
    'autotel.logs',
    logsProvider,
  );
  const errorsView = vscode.window.registerTreeDataProvider(
    'autotel.errors',
    errorsProvider,
  );
  ...
  const hoverSub = vscode.languages.registerHoverProvider(
    selector,
    hoverProvider,
  );
