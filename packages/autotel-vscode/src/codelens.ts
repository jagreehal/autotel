    ...
    for (const span of trace.spans) {
      ...
      if (existing) {
        ...
        byKey.set(key, {
          ...
          lineno,
          functionName:
            typeof attrs['code.function'] === 'string'
...
export class AutotelCodeLensProvider implements vscode.CodeLensProvider {
  ...
  provideCodeLenses(document: vscode.TextDocument): vscode.CodeLens[] {
    ...
    for (const fnStats of stats.values()) {
      ...
      if (
        !docPath.endsWith(fnStats.filepath) &&
        !fnStats.filepath.endsWith(docPath.replace(/\\/g, '/'))
      ) {
        continue;
      }
      ...
      const command: vscode.Command = {
        ...
        tooltip: fnStats.functionName
          ? `Function: ${fnStats.functionName}`
          : 'Open most recent span',
      };
...
export class AutotelHoverProvider implements vscode.HoverProvider {
  ...
  provideHover(
    ...
    for (const fnStats of stats.values()) {
      ...
      md.appendMarkdown(
        `**Autotel** · \`${fnStats.functionName ?? 'function'}\`\n\n`,
      );
