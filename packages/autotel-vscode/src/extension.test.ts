...
const {
  ...
  return {
    ...
    parseOtlpLogs: vi.fn(() => []),
    appendManyWithLimit: vi.fn((items: unknown[], incoming: unknown[]) => [
      ...items,
      ...incoming,
    ]),
    resolveTelemetryLimits: vi.fn(() => ({
      maxTraceCount: 10000,
      maxLogCount: 10000,
      maxMetricCount: 10000,
    })),
...
describe('activate', () => {
  beforeEach(() => {
    ...
    registerCommand.mockImplementation(() => {
      const [name, handler] = registerCommand.mock.lastCall as [
        string,
        (arg?: unknown) => void,
      ];
  ...
  it('detects autotel in a large monorepo regardless of scan position', async () => {
    ...
    const files = Array.from(
      { length: 250 },
      (_, i) => `/workspace/packages/p${i}/package.json`,
    );
  ...
  it('auto-start stays silent when the port is already in use', async () => {
    ...
    fakeServer.once.mockImplementation(
      (event: string, cb: (err: NodeJS.ErrnoException) => void) => {
        if (event === 'error')
          cb(Object.assign(new Error('in use'), { code: 'EADDRINUSE' }));
        return fakeServer;
      },
    );
    const context = extensionContext();
