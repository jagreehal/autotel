...
async function splitCredential(
  token: string | undefined,
): Promise<{ api: string; app: string } | undefined> {
...
function groupDatadogSpans(spans: DatadogSpan[]): TraceData[] {
  ...
  for (const [traceId, traceSpans] of byTraceId) {
    ...
    out.push({
      ...
      status: traceSpans.some((s) => s.status.code === 'ERROR')
        ? 'ERROR'
        : 'OK',
...
export const datadogAdapter: QueryAdapter = {
  ...
  async listServices(ctx) {
    const body = await ddFetch<DatadogServicesResponse>(
      ctx,
      'GET',
      '/api/v2/services',
    );
  ...
  async searchTraces(ctx, query: TraceQuery): Promise<TraceData[]> {
    ...
    const result = await ddFetch<DatadogSearchResponse>(
      ctx,
      'POST',
