...
function tempoTraceToTraceData(
  otlp: TempoOtlpTrace,
  fallbackId: string,
): TraceData {
  ...
  const startTime =
    allSpans.length > 0 ? Math.min(...allSpans.map((s) => s.startTime)) : 0;
  const endTime =
    allSpans.length > 0 ? Math.max(...allSpans.map((s) => s.endTime)) : 0;
  return {
...
export const tempoAdapter: QueryAdapter = {
  ...
  async searchTraces(ctx, query: TraceQuery): Promise<TraceData[]> {
    ...
    if (query.startMs)
      params.set('start', String(Math.floor(query.startMs / 1000)));
    if (query.endMs) params.set('end', String(Math.floor(query.endMs / 1000)));
    const search = await authedFetch<TempoSearchResponse>(
      ctx,
      `/api/search?${params}`,
    );
    const traceIds = (search.traces ?? [])
      .map((t) => t.traceID)
  ...
  async getTrace(ctx, traceId) {
    const body = await authedFetch<TempoOtlpTrace>(
      ctx,
