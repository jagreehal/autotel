...
function signozSpansToTrace(spans: SignozTraceSpan[]): TraceData | undefined {
  ...
  const translated: TraceData['spans'] = spans.map((s) => {
    ...
    return {
      ...
      status: {
        code: (s.statusCode ?? 0) >= 2 ? 'ERROR' : 'OK',
  ...
  return {
    ...
    service: String(
      root?.attributes?.['service.name'] ?? spans[0].serviceName ?? 'unknown',
    ),
  };
}
...
export const signozAdapter: QueryAdapter = {
  ...
  async listServices(ctx) {
    const body = await signozFetch<SignozServicesResponse>(
      ctx,
      'GET',
      '/api/v1/services',
    );
  ...
  async searchTraces(ctx, query: TraceQuery): Promise<TraceData[]> {
    ...
    if (query.service)
      filterItems.push({ key: 'serviceName', op: '=', value: query.service });
    if (query.errorsOnly)
      filterItems.push({ key: 'hasError', op: '=', value: true });
    const body = {
    ...
    const result = await signozFetch<{
      data?: { result?: Array<{ list?: SignozTraceSpan[] }> };
    }>(ctx, 'POST', '/api/v3/query_range', body);
