...
async function honeycombFetch<T>(
  ...
  if (!token)
    throw new Error(
      'Honeycomb API key missing. Run "Autotel: Set Remote Backend Credential" first.',
    );
...
function eventToTrace(event: HoneycombEvent, dataset: string): TraceData {
  ...
  const traceId = String(
    d['trace.trace_id'] ?? d['trace_id'] ?? `synth-${Math.random()}`,
  );
...
export const honeycombAdapter: QueryAdapter = {
  ...
  async listServices(ctx) {
    ...
    const datasets = await honeycombFetch<HoneycombDataset[]>(
      ctx,
      'GET',
      '/1/datasets',
    );
  ...
  async searchTraces(ctx, query: TraceQuery): Promise<TraceData[]> {
    ...
    if (!dataset)
      throw new Error(
        'Honeycomb requires a dataset. Set autotel.backend.dataset.',
      );
    const now = Date.now();
    ...
    const created = await honeycombFetch<{ id: string }>(
      ctx,
      'POST',
      `/1/queries/${dataset}`,
      queryBody,
