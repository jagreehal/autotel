      ...
      resourceSpans: [
        {
          resource: {
            attributes: [
              { key: 'service.name', value: { stringValue: 'dist-smoke' } },
            ],
          },
          scopeSpans: [
            {
...
assert.equal(
  ...
  'trace ID hex mismatch',
);
assert.equal(
  traces[0].spans[0].spanId,
  '0102030405060708',
  'span ID hex mismatch',
);
...
assert.doesNotThrow(
  () => decodeOtlpLogsRequest(Buffer.from([])),
  'logs decoder threw',
);
assert.doesNotThrow(
  () => decodeOtlpMetricsRequest(Buffer.from([])),
  'metrics decoder threw',
);

console.log(
  'dist ESM smoke OK — protobuf decoders load and decode from the built bundle',
);
