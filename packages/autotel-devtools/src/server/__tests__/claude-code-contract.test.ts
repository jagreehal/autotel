...
describe('Claude Code OTLP contract (recorded fixture)', () => {
  ...
  it('every emitted event.name is either handled or knowingly ignored', () => {
    ...
    const unknown = [...emitted].filter(
      (name) => !CLAUDE_CODE_KNOWN_EVENT_NAMES.has(name),
    );
    ...
    expect(
      unknown,
      `unhandled Claude Code events: ${unknown.join(', ')}`,
    ).toEqual([]);
  });
  ...
  it('the fixture still exercises the newly-modeled signals', () => {
    ...
    for (const name of [
      ...
      expect(
        emitted,
        `fixture missing ${name} — re-record with an MCP/plugin/hook-active session`,
      ).toContain(name);
    }
  });
});
