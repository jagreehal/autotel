...
describe('explainSpan', () => {
  ...
  it('detects tool decisions from a per-message finish reason', () => {
    const planning = explainSpan(
      span({
        messages: [
          { role: 'assistant', parts: [], finishReason: 'function_call' },
        ],
      }),
    );
    expect(planning.role).toBe('Model · planning');
  ...
  it('describes embeddings', () => {
    const e = explainSpan(
      span({ operation: 'embeddings', responseModel: 'text-embedding-3' }),
    );
  ...
  it('describes speech and transcription', () => {
    ...
    expect(explainSpan(span({ operation: 'transcription' })).role).toBe(
      'Transcription',
    );
  });
...
describe('buildTour', () => {
  ...
  it('does not mutate the input array', () => {
    const input = [
      span({ spanId: 'b', startMs: 2 }),
      span({ spanId: 'a', startMs: 1 }),
    ];
    buildTour(input);
