...
function normalizeMessageParts(raw: RawMessage): GenAiMessagePart[] {
  ...
  if (Array.isArray(raw.parts)) {
    return raw.parts.map((p): GenAiMessagePart => {
      ...
      if (type === 'text')
        return { kind: 'text', text: String(p.content ?? '') };
      if (type === 'image')
        return {
          kind: 'image',
          mediaType: 'image/*',
...
function normalizeMessage(raw: RawMessage): GenAiMessage {
  ...
  if (Array.isArray(raw.parts)) {
    ...
    if (
      parts.some(
        (p) => p.type === 'tool_call' || p.type === 'tool_call_response',
      )
    ) {
  ...
  if (Array.isArray(raw.tool_calls) && raw.tool_calls.length > 0) {
    msg.toolCalls = raw.tool_calls.map((tc): GenAiToolCall => ({
      ...
      arguments:
        parseJson(tc.function?.arguments) ?? tc.function?.arguments ?? {},
      type: tc.type,
    }));
  }
...
function readMessagesAttribute(
  attrs: Attrs,
  key: string,
...
function readToolDefinitions(attrs: Attrs): GenAiToolDef[] | undefined {
  ...
  >(
    attrs['gen_ai.tool.definitions'] ??
      attrs['gen_ai.orchestrator.agent.definitions'],
  );
...
function readStreaming(attrs: Attrs): GenAiStreaming | undefined {
  ...
  const timePerOutputChunkS = num(
    attrs['gen_ai.response.time_per_output_chunk'],
  );
  if (
...
function readGuardDetails(
  a: Attrs,
  defaultAction?: GenAiGuard['action'],
): GenAiGuard | undefined {
...
function readGuardAndWarnings(
  ...
  for (const ev of events ?? []) {
    ...
    else if (ev.name === 'gen_ai.guard.warning')
      apply(readGuardDetails(a, 'warn'));
...
export function toGenAiSpan(span: SpanData): GenAiSpan {
  ...
  const inputMessages =
    readMessagesAttribute(attrs, 'gen_ai.input.messages') ?? [];
  const outputMessages =
    readMessagesAttribute(attrs, 'gen_ai.output.messages') ?? [];
  ...
  const hasInputContent = messages.some(
  ...
  if (inputRef && !hasInputContent) {
    messages.push({
      role: 'user',
      parts: [{ kind: 'ref', ref: inputRef, direction: 'input' }],
    });
  }
  if (outputRef && !hasOutputContent) {
    messages.push({
      role: 'assistant',
      parts: [{ kind: 'ref', ref: outputRef, direction: 'output' }],
    });
  }
  ...
  if (messages.length === 0) {
      ...
      if (aiPromptBlob) {
        if (Array.isArray(aiPromptBlob.messages)) {
          ...
          if (aiPromptBlob.system) {
            messages.push({
              role: 'system',
              parts: [{ kind: 'text', text: aiPromptBlob.system }],
            });
          }
          if (aiPromptBlob.prompt) {
            messages.push({
              role: 'user',
              parts: [{ kind: 'text', text: aiPromptBlob.prompt }],
            });
          }
        }
      }
    ...
    const assistantToolCalls: GenAiToolCall[] | undefined = Array.isArray(
      aiResponseToolCalls,
    )
      ? aiResponseToolCalls
    ...
    if (
      ...
      const finishReason =
        strArray(attrs['gen_ai.response.finish_reasons'])?.[0] ??
        str(attrs['ai.response.finishReason']);
  ...
  if (messages.length === 0) {
    const pydanticMessages = parseJson<RawMessage[]>(
      attrs['pydantic_ai.all_messages'],
    );
  ...
  if (messages.length === 0) {
    ...
    for (let i = 0; i < 32; i++) {
      const role =
        str(attrs[`gen_ai.prompt.${i}.role`]) ??
        str(attrs[`llm.prompts.${i}.role`]);
      const content =
        attrs[`gen_ai.prompt.${i}.content`] ??
        attrs[`llm.prompts.${i}.content`];
      if (!role) break;
      legacy.push(normalizeMessage({ role, content }));
    }
    for (let i = 0; i < 32; i++) {
      const role =
        str(attrs[`gen_ai.completion.${i}.role`]) ??
        str(attrs[`llm.completions.${i}.role`]);
      const content =
        attrs[`gen_ai.completion.${i}.content`] ??
        attrs[`llm.completions.${i}.content`];
  ...
  for (const [k, v] of Object.entries(attrs)) {
    ...
    if (k.startsWith('gen_ai.prompt.') || k.startsWith('gen_ai.completion.'))
      continue;
    if (k.startsWith('llm.prompts.') || k.startsWith('llm.completions.'))
      continue;
    raw[k] = v;
  }
  ...
  return {
    ...
    handoff:
      handoffFrom || handoffTo
        ? { fromAgent: handoffFrom, toAgent: handoffTo }
        : undefined,
    guardrail:
      guardrailName || guardrailTriggered !== undefined
