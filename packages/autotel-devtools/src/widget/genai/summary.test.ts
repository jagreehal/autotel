...
describe('summarizeRun', () => {
  ...
  it('aggregates tokens across model calls', () => {
    const s = summarizeRun([
      span({
        usage: { inputTokens: 100, outputTokens: 20, reasoningOutputTokens: 5 },
      }),
      span({
        usage: { inputTokens: 50, outputTokens: 10, cacheReadInputTokens: 30 },
      }),
    ]);
  ...
  it('excludes aggregate/wrapper spans from model & token counts', () => {
    ...
    const s = summarizeRun([
      ...
      span({
        ...
        startMs: 3460,
        endMs: 5500,
        usage: { inputTokens: 209, outputTokens: 90 },
      }),
    ]);
    expect(s.modelCalls).toBe(2);
...
describe('groupRuns', () => {
  ...
  it('falls back to trace id when no conversation id', () => {
    const runs = groupRuns([
      row('a', 10, undefined, 'trX'),
      row('b', 20, undefined, 'trX'),
    ]);
  ...
  it('orders runs newest first by latest span', () => {
    const runs = groupRuns([row('a', 10, 'old'), row('b', 100, 'new')]);
