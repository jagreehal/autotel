    ...
    dataPoints: [
      {
        value: added,
        timestamp: 1000,
        attributes: { 'session.id': sessionId, type: 'added' },
      },
      {
        value: removed,
        timestamp: 1000,
        attributes: { 'session.id': sessionId, type: 'removed' },
      },
    ],
    resource: RESOURCE,
    scope: SCOPE,
...
export function sampleAgentSessions(): AgentSession[] {
  ...
  ingestAgentEvents(store, [
    ...
    event(a, 'plugin_loaded', tick(), {
      'plugin.name': 'context7',
      'plugin.version': '1.2.0',
    }),
    event(a, 'hook_execution_complete', tick(), {
