...
import SpanSearch from '../components/SpanSearch.svelte';
import {
  cleanup,
  render,
  fireEvent,
...
describe('SpanSearch with debounce', () => {
  ...
  it('debounces search input with 300ms default', async () => {
    ...
    render(SpanSearch, { props: { spans, onMatchesChange, onCurrentMatchChange } });
  ...
  it('clears matches when query is empty', async () => {
    ...
    render(SpanSearch, {
      props: { spans, onMatchesChange, onCurrentMatchChange: vi.fn() },
    });
    onMatchesChange.mockClear();
