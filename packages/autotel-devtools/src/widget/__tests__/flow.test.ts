...
describe('classifySpan', () => {
  it('detects AI tool calls by ai.toolCall.name', () => {
    const r = classifySpan(
      span({
        name: 'ai.toolCall',
        attributes: { 'ai.toolCall.name': 'calculate' },
      }),
    );
  ...
  it('treats SERVER spans as entry points', () => {
    expect(classifySpan(span({ kind: 'SERVER', name: 'POST /x' })).role).toBe(
      'entry',
    );
  });
  ...
  it('detects db spans', () => {
    expect(
      classifySpan(span({ name: 'q', attributes: { 'db.system': 'postgres' } }))
        .role,
    ).toBe('db');
  });
});
...
describe('buildFlowGraph', () => {
  ...
  it('surfaces plain functions alongside tools', () => {
    ...
    expect(
      nodes.find((n) => n.id === 'function|validateScenario'),
    ).toBeDefined();
  });
  ...
  it('sums per-span LLM metrics onto the collapsed node', () => {
    const metricsBySpanId = new Map([
      [
        '5000000000000001',
        { inputTokens: 10, outputTokens: 1, costUsd: 0.001 },
      ],
      [
        '5000000000000002',
        { inputTokens: 20, outputTokens: 2, costUsd: 0.002 },
      ],
    ]);
  ...
  it('leaves metrics undefined for nodes with no supplied metrics', () => {
    const { nodes } = buildFlowGraph(spans);
    expect(
      nodes.find((n) => n.id === 'tool|calculate')!.metrics,
    ).toBeUndefined();
  });
});
...
describe('layoutFlow', () => {
  ...
  it('handles an empty trace', () => {
    expect(layoutFlow(buildFlowGraph([]))).toMatchObject({
      nodes: [],
      edges: [],
    });
  });
