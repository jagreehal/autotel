...
- **Nothing reads the DOM through the shadow boundary.** The widget renders in a shadow root, so `document.activeElement` is the _host element_ rather than the focused input, and `document.getElementById` cannot see a row inside it at all. `isInputFocused` (`utils/keyboard.ts`) walks `shadowRoot.activeElement` down to the element that really has focus, and every global shortcut handler routes through it — without that walk each one fires while someone is typing, so `l` in the attribute filter switches the trace view and a digit switches tab. Query inside a container you hold a reference to (`scrollEl.querySelector`) rather than off `document`, and test a click against `event.composedPath()` rather than `element.contains(event.target)`.
