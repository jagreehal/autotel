...
describe('handleKey', () => {
  it('toggles view modes (t/l/v/E)', () => {
    ...
    expect(handleKey({ ...baseState, viewMode: 'span' }, 't').next.viewMode).toBe(
      'trace',
    );

    expect(handleKey(baseState, 'l').next.viewMode).toBe('log');
    expect(handleKey({ ...baseState, viewMode: 'log' }, 'l').next.viewMode).toBe(
      'trace',
    );

    expect(handleKey(baseState, 'v').next.viewMode).toBe('service-summary');
    expect(
      handleKey({ ...baseState, viewMode: 'service-summary' }, 'v').next.viewMode,
    ).toBe('trace');

    expect(handleKey(baseState, 'E').next.viewMode).toBe('errors');
    expect(handleKey({ ...baseState, viewMode: 'errors' }, 'E').next.viewMode).toBe(
      'trace',
    );
  });
  ...
  it('cycles status group with H', () => {
    ...
    const s2 = handleKey({ ...baseState, spanFilters: { statusGroup: s1 } }, 'H')
      .next.spanFilters.statusGroup;
    const s3 = handleKey({ ...baseState, spanFilters: { statusGroup: s2 } }, 'H')
      .next.spanFilters.statusGroup;
    const s4 = handleKey({ ...baseState, spanFilters: { statusGroup: s3 } }, 'H')
      .next.spanFilters.statusGroup;
