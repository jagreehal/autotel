...
- `scenario.ts`: flow-level scenario conformance: `ScenarioSpec` (events + cardinality + ancestor edges + completion boundary), `evaluateScenario`/`checkScenario` (three-state: conformant, non-conformant, or **incomplete**, where absence is definitive only after the completion boundary closes; errors and exceeded `max` fail fast while open; undeclared events are additive, never failing), `proposeScenario` (draft a contract from N recorded runs). Input `ScenarioSpan` is structurally compatible with `SerializedSpan` from `autotel/test-span-collector` (`peekTrace()` is the non-destructive polling read).
- `completeness.ts`: `scoreGenAiCompleteness(spans, options?)` → fidelity score for one GenAI trace over the ten fields root-cause analysis needs (llm input/output, model, tokens, cost, per-span latency, tool args/results, span tree, span count); 0.5 for partial (one-directional token usage, dangling parent ids, single-span trace). `options.notCapturable` names fields the deployment cannot capture at all (derive it from `autotel.coverage.unobserved`): those drop out of both `score` and `max`, so a blind spot never reads as forgotten instrumentation. `verdict` is `healthy` / `partial` / `unknown` / `invalid` — `unknown` outranks `partial` whenever a blind spot is declared, because knowing what you missed is the stronger position. `formatCompleteness` renders it. Same `ScenarioSpan` input as `scenario.ts`.
- `redaction.ts`: `highCardinalityKeys` / `isHighCardinalityKey` → redactor protect-list. Read the flag as _protect from redaction_, not _has many values_: setting `highCardinality` on a free-text field exempts the likeliest carrier of a pasted secret from the redactor. Ids yes, prose no.
