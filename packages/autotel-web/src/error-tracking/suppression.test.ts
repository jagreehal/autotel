...
describe('isSuppressed', () => {
  ...
  it('matches exact type', () => {
    const rules: SuppressionRule[] = [
      { key: 'type', operator: 'exact', value: 'ResizeObserver loop' },
    ];
    expect(isSuppressed(makeException('ResizeObserver loop', 'x'), rules)).toBe(
      true,
    );
  ...
  it('matches contains on value', () => {
    ...
    expect(isSuppressed(makeException('Error', 'Script error.'), rules)).toBe(
      true,
    );
  ...
  it('matches regex on value', () => {
    const rules: SuppressionRule[] = [
      { key: 'value', operator: 'regex', value: '^Loading chunk \\d+' },
    ];
    expect(
      isSuppressed(makeException('Error', 'Loading chunk 42 failed'), rules),
    ).toBe(true);
    expect(isSuppressed(makeException('Error', 'other error'), rules)).toBe(
      false,
    );
  });
  ...
  it('matches if any rule matches (OR logic)', () => {
    ...
    expect(
      isSuppressed(makeException('AbortError', 'request aborted'), rules),
    ).toBe(true);
    expect(isSuppressed(makeException('Error', 'Script error.'), rules)).toBe(
      true,
    );
    expect(isSuppressed(makeException('TypeError', 'other'), rules)).toBe(
      false,
    );
  });
  ...
  it('handles invalid regex gracefully', () => {
    const rules: SuppressionRule[] = [
      { key: 'value', operator: 'regex', value: '[invalid' },
    ];
